...
public class UsedCarNPC : MonoBehaviour, ISaleSystem
{
    ...
    public void RefreshSellSlots()
    {
        ...
        if (itemRaycast.IsHoldingLargeItem && itemRaycast.CurrentLargeItemData != null)
        {
            ...
            if (CanSell(item))
            {
                ...
                {
                    Debug.Log($"[UsedCarNPC] 이미지 설정 확인 - sprite: {ui.inventoryimage.sprite}, enabled: {ui.inventoryimage.enabled}");
                    
                    // 이미지가 설정되지 않았다면 수동으로 설정
                    if (ui.inventoryimage.sprite == null && item.icon != null)
                    {
                        Debug.LogWarning($"[UsedCarNPC] 이미지가 설정되지 않아 수동으로 설정합니다: {item.itemName}");
                ...
                {
                    Debug.LogError($"[UsedCarNPC] inventoryimage가 null입니다! GameObject: {go.name}");
                }
    ...
    public void ConfirmSell()
    {
        ...
        // 더러움 상태에 따른 가격 조정
        int cleanPrice = itemData.GetAdjustedSalePrice(); // 더러움 페널티 적용된 가격
