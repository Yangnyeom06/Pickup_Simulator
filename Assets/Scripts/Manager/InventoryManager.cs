...
public class InventoryManager : MonoBehaviour
{
    ...
    // 상점에서 구매한 아이템 추가 (통합 메서드)
    public bool AddStoreItem(StoreItemData storeItemData)
    {
        if (storeItemData == null)
        {
            Debug.LogError("storeItemData가 null입니다!");
            return false;
        }
        ...
        {
            // 빈 슬롯 확인 (상점 아이템과 간식 모두 확인)
            bool isSlotEmpty = (buyItemSlotList[i].currentShopItem == null && buyItemSlotList[i].currentSnackItem == null);
            
            if (isSlotEmpty)
            {
                storeItemData.slotNum = i; // 슬롯 인덱스를 그대로 사용
                
                // 아이템 타입에 따라 적절한 슬롯 설정 메서드 호출
                if (storeItemData.dataType == StoreItemDataType.ShopItem)
                {
                    buyItemSlotList[i].SetShopItem(storeItemData);
                    Debug.Log($"'{storeItemData.itemName}' 상점 아이템이 슬롯 {i}에 추가됨!");
                }
                else if (storeItemData.dataType == StoreItemDataType.Snack)
                {
                    buyItemSlotList[i].SetSnackItem(storeItemData);
                    Debug.Log($"'{storeItemData.snackName}' 간식이 슬롯 {i}에 추가됨!");
                }
                
                // 현재 날짜를 구매일로 설정 (간식의 경우 유통기한 계산용)
                int currentDay = DayManager.Instance != null ? DayManager.Instance.CalculateTotalDays() : 0;
                savedStoreItems.Add(new StoreItemInstanceData(storeItemData, storeItemData.slotNum, currentDay));
    ...
    // 하위 호환성을 위한 메서드들
    public bool AddShopItem(StoreItemData storeItemData)
    {
        return AddStoreItem(storeItemData);
    }

    public bool AddSnack(StoreItemData storeItemData)
    {
        return AddStoreItem(storeItemData);
    }
    ...
    public List<StoreItemData> GetAllStoreItems()
    {
        ...
        foreach (var slot in buyItemSlotList)
        {
            if (slot == null)
            {
                Debug.LogError("buyItemSlotList에 null이 들어있습니다!");
                continue;
            }

            // 상점 아이템과 간식 모두 포함
            if (slot.currentShopItem != null)
                result.Add(slot.currentShopItem);
    ...
    public List<StoreItemData> GetStoreItemsByType(StoreItemDataType itemType)
    {
        List<StoreItemData> result = new();
        foreach (var slot in buyItemSlotList)
        {
            ...
            StoreItemData item = null;
            if (slot.currentShopItem != null && slot.currentShopItem.dataType == itemType)
                item = slot.currentShopItem;
            else if (slot.currentSnackItem != null && slot.currentSnackItem.dataType == itemType)
                item = slot.currentSnackItem;
    ...
    // 하위 호환성을 위한 메서드들
    public List<StoreItemData> GetPickedUpSnacks()
    {
        return GetStoreItemsByType(StoreItemDataType.Snack);
    }

    public List<StoreItemData> GetAllSnacks()
    {
        return GetStoreItemsByType(StoreItemDataType.Snack);
    }
    ...
    {
        return GetStoreItemsByType(StoreItemDataType.ShopItem);
    }
    ...
    public void ClearSlotContents(InventorySlotData slot)
    {
        ...
        {
            Debug.LogWarning("해당 슬롯이 slotList에 없습니다!");
        }
    }
